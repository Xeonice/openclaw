...
export function applySparkProviderConfig(cfg: OpenClawConfig): OpenClawConfig {
  const models = { ...cfg.agents?.defaults?.models };
  models[SPARK_DEFAULT_MODEL_REF] = {
    ...models[SPARK_DEFAULT_MODEL_REF],
    alias: models[SPARK_DEFAULT_MODEL_REF]?.alias ?? "Spark",
  };

  const providers = { ...cfg.models?.providers };
  const existingProvider = providers.spark;
  const defaultProvider = buildSparkProvider();
  const existingModels = Array.isArray(existingProvider?.models) ? existingProvider.models : [];
  const defaultModels = defaultProvider.models ?? [];
  const hasDefaultModel = existingModels.some((model) => model.id === SPARK_DEFAULT_MODEL_ID);
  const mergedModels =
    existingModels.length > 0
      ? hasDefaultModel
        ? existingModels
...
export function applySparkConfig(cfg: OpenClawConfig): OpenClawConfig {
  const next = applySparkProviderConfig(cfg);
  const existingModel = next.agents?.defaults?.model;
  return {
    ...next,
    agents: {
      ...
      defaults: {
        ...
        model: {
          ...(existingModel && "fallbacks" in (existingModel as Record<string, unknown>)
            ...
            : undefined),
          primary: SPARK_DEFAULT_MODEL_REF,
        },
      },
    },
  };
